...
func copyMatrix44(src: [Float32], dst: inout Matrix4x4) {
...
func copyMatrix33(src: [Float32], dst: inout Matrix3x3) {
...
func matrix44MakeLookAt(eye: Vector3, center: Vector3, up: Vector3) -> Matrix4x4{
    ...
    var u: Vector3 = uv.cross(other: n)
        u.normalize()
    
    let v: Vector3 = n.cross(other: u)
    ...
    let m = [u.x, v.x, n.x, 0.0,
             u.y, v.y, n.y, 0.0,
             u.z, v.z, n.z, 0.0,
             un.dot(v: ev), vn.dot(v: ev), nn.dot(v: ev), 1.0]
